...
MLflow is external to the dashboard application and should be opened separately.
...
> Note: Local dev uses port `8010` for the FastAPI service, while Docker Compose maps the API to `8000`. The dashboard now tries `8010` first, then `8000` if needed.
